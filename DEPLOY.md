...
## 🔑 예비 API 키 (한도 초과 대비, 강력 추천)

무료 등급은 한도가 있어서, 글을 몰아 쓰면 막힙니다. **다른 구글 계정으로 키를 하나 더 발급**받아
예비키로 넣어두면, 첫 키가 한도에 걸릴 때 **자동으로 다음 키를 사용**합니다.

Vercel → **Settings → Environment Variables** 에 아래를 추가하면 됩니다(원하는 만큼):
- `GEMINI_API_KEY` (기본)
- `GEMINI_API_KEY_2` (예비 1)
- `GEMINI_API_KEY_3` (예비 2)

추가/변경 후에는 **Redeploy** 해야 적용됩니다. (로컬은 `.env.local`에 같은 이름으로 넣으면 됩니다.)
